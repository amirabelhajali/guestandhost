...
//            }
    ...
    function MesReservationsAPayerAction()
    {
        ...
        return $this->render('MyAppUserBundle:reservation:MesReservationsAPayer.html.twig',array('r'=>$reservations,'img'=>$images));

    }

    function PayerAction($id)
    {
        $msg='';
        $paiment1 = new Paiment();
        $paiment2 = new Paiment();
        $paiment3 = new Paiment();
        $em = $this->getDoctrine()->getManager();
        $reservation = $em->getRepository('MyAppUserBundle:Reservation')->find($id);

        $userCo = $em->getRepository('MyAppUserBundle:User')->find($this->getUser()->getId());
        $annonce = $em->getRepository('MyAppUserBundle:Annonce')->find($reservation->getIdAnnonce());
        $userAnnonce = $em->getRepository('MyAppUserBundle:User')->find($annonce->getIdUser());
        $Admin = $em->getRepository('MyAppUserBundle:User')->find(74);
        if($userCo->getSolde() >= $reservation->getPrix())
        {
            $soldeAdmin = (($reservation->getPrix()*30)/100);
            $userCo->setSolde($userCo->getSolde() - ($reservation->getPrix()+$soldeAdmin));
            $userAnnonce->setSolde($userAnnonce->getSolde() + $reservation->getPrix());
            $Admin->setSolde($Admin->getSolde() + $soldeAdmin);

            $reservation->setEtat('Payee');
            $paiment1->setIdUser($annonce->getIdUser());
            $paiment1->setHistorique('Chargement compte de');
            $paiment1->setPrix($reservation->getPrix());
            $em->persist($paiment1);

            $paiment2->setIdUser($reservation->getIdUser());
            $paiment2->setHistorique('Retrait de');
            $paiment2->setPrix($reservation->getPrix()+ $soldeAdmin);
            $em->persist($paiment2);


            $paiment3->setIdUser($Admin);
            $paiment3->setHistorique('admin');
            $paiment3->setPrix($soldeAdmin);
            $em->persist($paiment3);

            $em->flush();
            $this->get('ras_flash_alert.alert_reporter')->addSuccess('Paiment Effectuer avec sccées;');
        }else{
            $msg='Vous ne pouvez pas effectuer cette reservation , solde insuffisant,Veuillez recharger votre compte';
            $this->get('ras_flash_alert.alert_reporter')->addError($msg);
        }
        $em->persist($reservation);
        $em->flush();
        return $this->redirectToRoute('Mes reservations a payer');

    }

    function HistoriquePaimentAction()
    {
        $id=$this->getUser()->getId();
        $em = $this->getDoctrine()->getManager();
        $paiments = $em->getRepository('MyAppUserBundle:Paiment')->findBy(array('idUser'=>$id));
        return $this->render('MyAppUserBundle:reservation:HistoriquePaiment.html.twig',array('p'=>$paiments));
    }

    function GeneratePdfAction($id)
    {
        $em=$this->getDoctrine()->getManager();
        $reservation=$em->getRepository('MyAppUserBundle:Reservation')->findOneBy(array('id'=>$id));

            $images[$reservation->getId()]=$em->getRepository('MyAppUserBundle:Image')->findBy(array('idAnnonce'=>$reservation->getIdAnnonce()));

        $html = $this->renderView('MyAppUserBundle:reservation:Facture.html.twig', array(
            'r'  => $reservation,'img'=>$images
        ));
    ...
    function AfficherFavoriesAction(Request $request)
    {
        ...
        return $this->render('MyAppUserBundle:reservation:Favoris.html.twig',array('f'=>$Favories,'pagination' => $pagination,'img'=>$images));
